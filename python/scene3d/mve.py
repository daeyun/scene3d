...
_cpp_binary_root = path.realpath(path.join(path.dirname(path.realpath(__file__)), '../../cpp/cmake-build-release/'))
_mve_root = path.realpath(path.join(path.dirname(path.realpath(__file__)), '../../cpp/third_party/repos/mve'))
...
def save_as_mve_views(scene_dir, masked_depth_images: np.ndarray, ortho_cameras: typing.Sequence[camera.OrthographicCamera], focal_length, cam_scale=1.0):
    """
...
def convert_mve_views_to_meshes(scene_dir):
    executable = path.join(_cpp_binary_root, 'cpp/apps/depth2mesh_perspective')
