...
class MultiLayerDepthAndSegmentation(data.Dataset):
    ...
    def __init__(self, train=True, first_n=None, rgb_scale=1.0, subtract_mean=True, image_hw=(240, 320)):
        self.filename_prefixes = io_utils.read_lines_and_strip(path.join(config.scene3d_root, 'v4/renderings.txt'))
    ...
    def __getitem__(self, index):
        ...
        bin_filenames = [
            path.join(config.scene3d_root, 'v4/renderings', example_name + '.bin'),
            path.join(config.scene3d_root, 'v4/renderings', example_name + '_bg.bin'),
            path.join(config.scene3d_root, 'v4/renderings', example_name + '_c.bin'),
            path.join(config.scene3d_root, 'v4/renderings', example_name + '_bg_c.bin'),
            path.join(config.scene3d_root, 'v4/renderings', example_name + '_td.bin'),
            path.join(config.scene3d_root, 'v4/renderings', example_name + '_td_prim.bin'),
            path.join(config.scene3d_root, 'v4/renderings', example_name + '_prim.bin'),
        ]
        ...
        return example_name, in_rgb, gt_ml_depth, count_image, d0, gt_ml_category, foreground_prim, top_down_foreground, top_down_foreground_prim
